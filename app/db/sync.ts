// @ts-check
import { SyncDatabaseChangeSet, synchronize, SyncTableChangeSet, Timestamp } from "@nozbe/watermelondb/sync"
            ...
            if (changes[type][action]) {
                changes[type][action].forEach(record => {
                    ...
                    if (record.date_of_birth) {
                        record.date_of_birth = new Date(record.date_of_birth || defaultDate).toISOString().split('T')[0];
                    }
